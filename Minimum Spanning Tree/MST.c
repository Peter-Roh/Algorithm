...
void Prim(Graph* G, Vertex* StartVertex, Graph* MST)
{
	...
	while(! PQ_IsEmpty(PQ))
	{
		...
		{
			Veretx* TargetVertex = CurrentEdge->Target;

			if(Fringes[TargetVertex->Index] == NULL && CurrentEdge->Weight < Weights[TargetVertex->Index])
			{
				PONode NewNode = {CurrentEdge->Weight, TargetVertex};
...
void Kruskal(Graph* G, Graph* MST)
{
	...
	Vertex** MSTVertices = (Vertex**)malloc(sizeof(Vertex*) * G->VertexCount);
	DisjointSet** VeretexSet = (DisjointSet**)malloc(sizeof(DisjointSet*) * G->VertexCount);
	...
	while(CurrentVertex != NULL)
	{
		...
		VeretexSet[i] = DS_MakeSet(CurrentVertex);
	...
	while(! PQ_IsEmpty(PQ))
	{
		...
		if(DS_FindSet(VertexSet[FromIndex]) != DS_FindSet(VertexSet[ToIndex]))
		{
			...
			DS_UnionSet(VertexSet[FromIndex], VeretexSet[ToIndex]);
		}
	}
